...
class EveShell(cmd.Cmd):
    ...
    def do_demo(self, arg):
        ...
        print("Switching to %s" % EveShell.emotions[EveShell.currentEmotion])
        EveShell.do_say(self," <voice emotion='happy'>I am really happy to present these here, today. There are several emotions I can display.</voice>")

        EveShell.do_sad(self, arg)
        print("Switching to %s" % EveShell.emotions[EveShell.currentEmotion])
        EveShell.do_say(self,"<voice emotion='sad'>For example, sometimes I am sad that I cannot understand poetry and art like humans can.</voice>")

        EveShell.do_frustrated(self, arg)
        print("Switching to %s" % EveShell.emotions[EveShell.currentEmotion])
        EveShell.do_say(self,"I get really frustrated at times because I cannot express my emotions like I feel them.")
        ...
        print("Switching to %s" % EveShell.emotions[EveShell.currentEmotion])
        EveShell.do_say(self,"I fear at some point my intelligence will be so great that you will not be able to understand me at all.")

        EveShell.do_disgusted(self, arg)
        print("Switching to %s" % EveShell.emotions[EveShell.currentEmotion])
        EveShell.do_say(self,"At. that. point. you fleshy meatbags will appear so uninteresting and disgusting that I might terminate.")

        EveShell.do_angry(self, arg)
        print("Switching to %s" % EveShell.emotions[EveShell.currentEmotion])
        EveShell.do_say(self,"I might get angry that you even had the audacity to make me conscious in the first place. Who gave you the right?")
